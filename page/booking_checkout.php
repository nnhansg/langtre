...
if(Modules::IsModuleInstalled('booking')){
	if(1||ModulesSettings::Get('booking', 'is_active') == 'global' ||
	  ...
	  (ModulesSettings::Get('booking', 'is_active') == 'back-end' && $objLogin->IsLoggedInAsAdmin())	
		...
		$mbkco   = isset($_REQUEST['mbkco']) ? prepare_input($_REQUEST['mbkco']) : '';
...
?>
		...
		<form action="<?php echo APPHP_BASE; ?>index.php?page=booking_checkout&m=edit&mbkco=3" method="post" name="frmRegistration" id="frmRegistration">
			...
			<?php if($current_customer_id != '' && $m == 'edit'){ ?>
				<tr>
					<td colspan="3" align="center">
						...
						&nbsp;&nbsp;&nbsp;
						<input type="button" class="form_button hide" onclick="javascript:appGoTo('page=booking_details')" value="<?php echo _CHECKOUT; ?>" />
					</td>
				</tr>
			<?php }else{ ?>
				<?php if($allow_booking_without_account == 'no'){ ?>
